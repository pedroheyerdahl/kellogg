...
```
Once the data is loaded into BQ, the first transformation step comprises a regex function to remove special characters from currency values, such as "$" and "-". This same function transform zeroed values to NULL. Next, the external connection data is materialized into a native bq table, to improve performance.
